...
const initiateBreaker = createCircuitBreaker(
  (url: unknown, payload: unknown, config: unknown) =>
    payunitClient.post(url as string, payload, config as object),
  'payunit-initiate'
);

const verifyBreaker = createCircuitBreaker(
  (url: unknown, config: unknown) =>
    payunitClient.get(url as string, config as object),
  'payunit-verify'
);
...
export class PayUnitService {
  ...
  async initiatePayment(dto: InitiatePaymentDto): Promise<PaymentResult> {
    ...
    try {
      ...
      // Notifier les clients SSE que le paiement est prêt
  ...
  async handleCallback(body: PayUnitCallbackBody): Promise<boolean> {
    ...
    // Notifier les clients SSE du statut final
